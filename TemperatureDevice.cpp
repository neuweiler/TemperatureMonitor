...
static OneWire ds(SENSOR_PIN); // DS18B20 Temperature chip i/o

/**
 * Constructor
 */
TemperatureDevice::TemperatureDevice(byte addr[]) {
 ...
 */
TemperatureDevice::DeviceType TemperatureDevice::getType() {
	return type;
}
 ...
 */
char *TemperatureDevice::getTypeStr() {
	switch (type) {
	case TemperatureDevice::DS18S20:
		return "DS18S20";
		break;
	case TemperatureDevice::DS18B20:
		return "DS18B20";
		break;
	case TemperatureDevice::DS1822:
		return "DS1822";
		break;
	default:
		return "unknown";
 ...
 */
byte *TemperatureDevice::getAddress() {
	return address;
}
 ...
 */
void TemperatureDevice::setResolution(byte resolution) {
 ...
 */
void TemperatureDevice::prepareData() {
	ds.reset();
 ...
 */
void TemperatureDevice::retrieveData() {
	byte data[9];
 ...
 */
float TemperatureDevice::getTemperatureCelsius() {
 ...
 */
float TemperatureDevice::getTemperatureFahrenheit() {
 ...
 */
void TemperatureDevice::resetSearch() {
	ds.reset_search();
}
 ...
 */
TemperatureDevice *TemperatureDevice::search() {
	byte addr[8];
	...
	if (ds.search(addr)) {
		...
		return new TemperatureDevice(addr);
	}
